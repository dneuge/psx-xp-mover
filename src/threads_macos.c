...
int mtx_init(mtx_t *mutex, int type) {
	...
	if (err) {
		printf("[threads_macos] pthread_mutexattr_init error: %d\n", err);
		return thrd_error;
	}
	...
	if (type & mtx_recursive) {
		...
		if (err) {
			printf("[threads_macos] pthread_mutexattr_settype PTHREAD_MUTEX_RECURSIVE error: %d\n", err);
			goto end;
		}
	}
	...
	if (err) {
		printf("[threads_macos] pthread_mutex_init error: %d\n", err);
	}

end:
	fin_err = pthread_mutexattr_destroy(&attr);
	if (fin_err) {
		printf("[threads_macos] pthread_mutexattr_destroy error: %d\n", fin_err);
	}
...
void mtx_destroy(mtx_t *mutex) {
	...
	if (err) {
		printf("[threads_macos] pthread_mutex_destroy error: %d\n", err);
	}
}
...
int mtx_lock(mtx_t *mutex) {
	...
	if (err) {
		printf("[threads_macos] pthread_mutex_lock error: %d\n", err);
	}
...
int mtx_unlock(mtx_t *mutex) {
	...
	if (err) {
		printf("[threads_macos] pthread_mutex_unlock error: %d\n", err);
	}
...
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg) {
	...
	if (err) {
		printf("[threads_macos] pthread_create error: %d\n", err);
		free(wrapped);
	}
...
int thrd_join(thrd_t thr, int *res) {
	...
	if (err) {
		printf("[threads_macos] pthread_join error: %d\n", err);
		return thrd_error;
	}
